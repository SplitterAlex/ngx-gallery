...
export class GallerySliderComponent implements OnInit, OnChanges, OnDestroy {
  ...
  get zoom() {
    return {
      transform: `perspective(50px) translate3d(0, 0, ${-this.config
        .zoomOut}px)`
    };
  }
      ...
      case SlidingDirection.Horizontal:
        return {
          transform: `translate3d(${-(
            this.state.currIndex * this._el.nativeElement.offsetWidth
          ) + state.value}px, 0, 0)`,
      ...
      case SlidingDirection.Vertical:
        return {
          transform: `translate3d(0, ${-(
            this.state.currIndex * this._el.nativeElement.offsetHeight
          ) + state.value}px, 0)`,
          width: '100%',
  ...
  private verticalPan(e) {
    if (!(e.offsetDirection & Hammer.DIRECTION_VERTICAL)) {
      return;
    }
    if (e.overallVelocityY > 0.3) {
      this.prev();
    } else if (e.overallVelocityY < -0.3) {
      this.next();
    } else {
      if (
        e.deltaY <=
        -this._el.nativeElement.offsetHeight / this.config.panSensitivity
      ) {
        this.next();
      } else if (
        e.deltaY >=
  ...
  private horizontalPan(e) {
    if (!(e.offsetDirection & Hammer.DIRECTION_HORIZONTAL)) {
      return;
    }
    // if (e.overallVelocityX > 0.3 && Math.abs(e.overallVelocityY) < 0.15) {
    if (e.overallVelocityX > 0.3 && Math.abs(e.deltaY) < 50) {
      this.prev();
    } else if (
      e.overallVelocityX < -0.3 &&
      Math.abs(e.deltaY) < 50
      // Math.abs(e.overallVelocityY) < 0.15
    ) {
      this.next();
    } else {
      if (
        e.deltaX <=
        -this._el.nativeElement.offsetWidth / this.config.panSensitivity
      ) {
        this.next();
      } else if (
        e.deltaX >=
