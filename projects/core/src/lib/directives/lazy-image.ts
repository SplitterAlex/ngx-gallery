...
export class LazyImage implements OnChanges, OnDestroy {
  ...
  constructor() {
    this._loaderSub$ = this._imageLoader$
      .pipe(switchMap((imageSrc: string) => this.nativeLoader(imageSrc)))
      .subscribe();
  }
  ...
  nativeLoader(url: string): Observable<any> {
    ...
    const loadError = fromEvent(img, 'error').pipe(
      tap(() =>
        this.error.emit(new Error(`[lazyImage]: The image ${url} did not load`))
      )
    );
